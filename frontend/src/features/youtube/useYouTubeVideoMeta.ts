...
type CacheShape = {
  updatedAt: number;
  items: Record<string, YouTubeVideoMeta>;
};
...
function loadCache(): CacheShape {
  try {
    ...
    return {
      ...
      items: parsed.items && typeof parsed.items === "object" ? (parsed.items as any) : {},
    };
  } catch {
...
async function fetchMeta(ids: string[], apiKey: string): Promise<Record<string, YouTubeVideoMeta>> {
  ...
  const out: Record<string, YouTubeVideoMeta> = {};
...
export function useYouTubeVideoMeta(urls: string[]) {
  ...
  const [metaById, setMetaById] = useState<Record<string, YouTubeVideoMeta>>(() => loadCache().items);
