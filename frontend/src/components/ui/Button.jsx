...
export function Button({ className, variant = "primary", size = "md", type = "button", whileHover, whileTap, transition, ...props }) {
    const reducedMotion = useReducedMotion();
    const disabled = Boolean(props.disabled);
    const hoverAnim = reducedMotion ? undefined : { scale: 1.04, y: -2 };
    const tapAnim = reducedMotion ? undefined : { scale: 0.96, y: 0 };
    return (<motion.button type={type} whileHover={disabled ? undefined : (whileHover ?? hoverAnim)} whileTap={disabled ? undefined : (whileTap ?? tapAnim)} transition={transition ?? { type: "spring", stiffness: 500, damping: 22 }} className={cn("inline-flex items-center justify-center font-semibold transition-all duration-200", "focus-ring disabled:opacity-50 disabled:pointer-events-none", variantClasses[variant], sizeClasses[size], className)} {...props}/>);
}
