...
export function TopicAutocomplete({ value, subject, placeholder = "Type to search topics…", onChange, onSelect, }) {
    ...
    return (<div ref={wrapRef} className="relative w-full">
      ...
      <div className="relative flex items-center">
        ...
        <input ref={inputRef} type="text" value={value} autoComplete="off" spellCheck={false} placeholder={placeholder} className="w-full rounded-2xl border border-white/10 bg-white/10 py-2.5 pl-9 pr-9 text-sm font-semibold text-ink-100 placeholder:text-ink-400 transition hover:bg-white/15 focus:outline-none focus:ring-2 focus:ring-byjus-500/50" onChange={handleChange} onFocus={() => setOpen(true)} onKeyDown={handleKeyDown}/>
        {value ? (<button type="button" tabIndex={-1} aria-label="Clear" className="absolute right-3 text-ink-400 hover:text-ink-100 transition" onMouseDown={(e) => {
                e.preventDefault();
                onChange("");
                setOpen(true);
            ...
            <X className="h-4 w-4"/>
          </button>) : (<ChevronDown className={`pointer-events-none absolute right-3 h-4 w-4 text-ink-400 transition-transform duration-150 ${open ? "rotate-180" : ""}`}/>)}
      </div>

      {/* Dropdown */}
      <AnimatePresence>
        {open && suggestions.length > 0 && (<motion.div key="ac-dropdown" initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: 0.1 }} 
        ...
        className="absolute left-0 right-0 top-full z-[200] mt-1 overflow-hidden rounded-2xl border border-white/15 bg-ink-900/98 shadow-2xl backdrop-blur-xl">
            ...
            <ul ref={listRef} className="max-h-52 overflow-y-auto py-1">
              {suggestions.map((topic, i) => (<li key={topic}>
                  <button type="button" tabIndex={-1} onMouseDown={(e) => {
                    e.preventDefault(); // keep input focused
                    pick(topic);
                }} onMouseEnter={() => setActiveIdx(i)} className={`flex w-full items-center gap-2.5 px-3 py-2.5 text-left text-sm transition ${i === activeIdx
