    ...
    return (<div className="space-y-5">
      ...
      {myStats && (<Reveal delay={0.05}>
          <Card className="p-5">
            ...
            {myStats.badges.length > 0 && (<div className="mt-3 flex flex-wrap gap-1.5">
                {myStats.badges.map((b) => (<Badge key={b} className="bg-byjus-600/20 border-byjus-500/30 text-byjus-300 text-[10px]">{b}</Badge>))}
              </div>)}
          </Card>
        </Reveal>)}
      ...
      {/* Leaderboard */}
      <Reveal delay={0.1}>
        {loading ? (<div className="flex justify-center py-8"><Loader2 className="h-6 w-6 animate-spin text-byjus-400"/></div>) : entries.length === 0 ? (<Card className="p-8 text-center">
            ...
            <motion.div className="space-y-2" variants={staggerContainer} initial="hidden" animate="show">
              {entries.map((e) => {
                ...
                return (<motion.div key={e.student_id} variants={staggerItem} whileHover={{ x: 4, scale: 1.01 }} transition={{ type: "spring", stiffness: 400, damping: 25 }} className={`flex items-center gap-3 rounded-2xl border p-3 ${rankBg(e.rank)} ${isMe ? "ring-1 ring-byjus-400/40" : ""}`}>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-3 mt-0.5">
                        ...
                        {e.badges.slice(0, 2).map((b) => (<span key={b} className="text-[10px] text-ink-500">{b}</span>))}
                      </div>
                    </div>
