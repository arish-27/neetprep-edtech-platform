...
function ToggleRow({
  ...
  return (
    <div className="flex items-start justify-between gap-4 rounded-3xl border border-white/10 bg-white/5 p-4 shadow-soft">
      <div>
      ...
      <button
        type="button"
        className={cn(
          "relative h-8 w-14 rounded-full border shadow-soft transition focus-ring",
          value ? "bg-byjus-600 border-byjus-600" : "bg-white/10 border-white/10",
      ...
      >
        <motion.div
          className="absolute top-0.5 h-7 w-7 rounded-full bg-white shadow-soft"
          initial={false}
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="p-5">
          ...
          <div className="mt-4 space-y-3">
            <ToggleRow
              label="Dark mode"
              desc="Reduce glare and study comfortably at night."
              value={theme === "dark"}
            ...
            <ToggleRow
              label="Notifications"
              desc="Get reminders for live classes and daily goals."
              value={notifications}
              onChange={setNotifications}
            />
            <ToggleRow
              label="Sound effects"
              desc="Play subtle sounds for interactions (demo toggle)."
              value={sound}
              onChange={setSound}
            />
            <ToggleRow
              label="Mobile data mode"
              desc="Prefer lower bandwidth video quality (demo toggle)."
              value={mobileData}
                ...
                {SPEEDS.map((s) => {
                  ...
                  return (
                    <button
                      key={s}
                      type="button"
                      onClick={() => setDefaultPlaybackRate(s)}
                      className={cn(
