...
import { api, type ApiQuizPublic } from "@/lib/api";
...
export function QuizAttemptScreen() {
  ...
  useEffect(() => {
    ...
    if (current?.type === "quiz" && current.questionOrder.length > 0) return;
  ...
  const submit = async (autoSubmitted: boolean) => {
    if (!quiz || !active) return;
    ...
    for (const id of order) {
      ...
      if (typeof picked === "number") submitAnswers[id] = picked;
    }
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[1.1fr_.9fr]">
      <Card className="p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              ...
              className="h-10 rounded-2xl"
              onClick={() => toggleMarkForReview(quizId, q.id)}
            >
        ...
        {submitError ? (
          <div className="mt-4 rounded-2xl border border-red-400/30 bg-red-500/10 px-4 py-3 text-sm font-semibold text-red-700 dark:text-red-100">
            {submitError}
          </div>
        ) : null}
              ...
              {q.options.map((opt, i) => {
                ...
                return (
                  <motion.button
                    key={`${q.id}_${i}`}
                    type="button"
                    onClick={() => answer(quizId, q.id, i)}
                    className={cn(
                      "w-full text-left rounded-2xl border px-4 py-3 shadow-soft transition focus-ring",
                      cls,
                    )}
                    animate={
                      showWrong
        ...
        <div className="mt-5 flex items-center justify-between gap-2">
          ...
          {idx === total - 1 ? (
            <Button className="h-11 rounded-2xl" onClick={() => void submit(false)}>
              Submit Quiz
            </Button>
          ) : (
