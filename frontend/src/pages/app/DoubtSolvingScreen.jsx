        ...
        <Card className="p-5 flex flex-col min-h-[520px]">
          ...
          <div className="mt-5 flex-1 space-y-3 overflow-auto pr-1">
            <AnimatePresence initial={false}>
              {messages.map((m) => (<motion.div key={m.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.22, ease: "easeOut" }} className={cn("max-w-[85%] rounded-3xl border px-4 py-3 shadow-soft", m.from === "student"
          ...
          <div className="mt-4 flex gap-2">
            <button type="button" className="grid h-11 w-11 place-items-center rounded-2xl border border-white/10 bg-white/10 shadow-soft hover:bg-white/15 focus-ring" title="Attach image (placeholder)">
              <Image className="h-5 w-5 text-ink-200"/>
            </button>
            <input value={text} onChange={(e) => setText(e.target.value)} className="h-11 w-full rounded-2xl border border-white/10 bg-white/10 px-4 text-sm font-semibold text-ink-100 shadow-soft focus-ring placeholder:text-ink-400" placeholder="Type your doubt..." onKeyDown={(e) => {
            if (e.key === "Enter")
                send();
        }}/>
