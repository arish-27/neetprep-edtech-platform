    ...
    return (<div className="grid gap-4 lg:grid-cols-[1.2fr_.8fr]">
      <Reveal>
        <Card className="p-5">
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <Badge className="bg-white/10 border-white/10 text-ink-200">
                <motion.span animate={{ opacity: [1, 0.3, 1] }} transition={{ duration: 1.2, repeat: Infinity, ease: "easeInOut" }} className="flex items-center gap-1.5">
          ...
          {/* ── Error ── */}
          {error && (<div className="mt-4 rounded-2xl border border-red-400/30 bg-red-500/10 px-4 py-3 text-sm font-semibold text-red-700 dark:text-red-200">
              {error}
            </div>)}

          {/* ── Video Box — dark purple bg, ALL text pure white ── */}
          <div className="mt-5 overflow-hidden rounded-3xl" style={{
            ...
            <div className="relative w-full pt-[56.25%]">
              <div className="absolute inset-0">
                {youtubeId ? (<ReactPlayer url={`https://www.youtube.com/watch?v=${youtubeId}`} playing controls width="100%" height="100%" config={{ youtube: { playerVars: { autoplay: 1, modestbranding: 1, rel: 0, playsinline: 1 } } }}/>) : (
        /* ── Waiting screen — everything WHITE ── */
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 px-6 text-center">
                    {/* LIVE pill */}
                    <div className="inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-xs font-extrabold" style={{
                background: "rgba(239,68,68,0.2)",
                border: "1px solid rgba(239,68,68,0.6)",
                color: "#FFFFFF",
            }}>
                      <span className="h-2 w-2 rounded-full bg-red-500 animate-pulse" style={{ boxShadow: "0 0 10px rgba(239,68,68,1)" }}/>
                      LIVE
                    </div>

                    {/* Main heading — pure white, large */}
                    <p className="text-2xl font-extrabold" style={{ color: "#FFFFFF", textShadow: "0 2px 20px rgba(0,0,0,0.6)" }}>
                      Join to start watching
                    </p>

                    {/* Sub text — white, clearly readable */}
                    <p className="text-sm font-semibold max-w-xs" style={{ color: "#FFFFFF", opacity: 0.9 }}>
                      Video plays inside the app. If your session expired, you'll be asked to log in again.
                    </p>

                    {/* Animated purple dots */}
                    <div className="flex gap-2 mt-1">
                      {[0, 1, 2].map((i) => (<motion.div key={i} className="h-2 w-2 rounded-full" style={{ background: "#A78BFA" }} animate={{ opacity: [0.3, 1, 0.3], scale: [0.8, 1.4, 0.8] }} transition={{ duration: 1.4, repeat: Infinity, delay: i * 0.25 }}/>))}
                    </div>
                  </div>)}
              </div>
              ...
              <div className="absolute inset-0 pointer-events-none">
                <div className="absolute top-3 right-3 rounded-xl px-3 py-1.5 text-xs font-bold" style={{
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <motion.div animate={!youtubeId && !joining ? {
      ...
      <Reveal delay={0.05}>
        <Card className="p-5">
          ...
          <div className="mt-4">
            <textarea className="w-full min-h-[110px] rounded-3xl border border-white/10 bg-white/10 px-4 py-3 text-sm font-semibold text-ink-900 dark:text-ink-100 focus-ring placeholder:text-ink-400" placeholder="Type your doubt..."/>
