...
export function VideoPlayerScreen() {
    ...
    const [playlistLessons, setPlaylistLessons] = useState([]);
    ...
    return (<div className="space-y-4">
        ...
        <Card className="p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="min-w-0">
              <Link to="/app/recorded-classes" className="inline-flex items-center gap-1 text-sm font-bold text-byjus-300 hover:underline">
      ...
      {accessLoading ? (<Card className="p-5">
          <Skeleton className="h-6 w-40"/>
          <Skeleton className="mt-4 h-56 w-full rounded-3xl"/>
          <Skeleton className="mt-4 h-4 w-2/3"/>
        </Card>) : accessAllowed ? (
        // Video section — from uploaded code: scale 0.8 → 1 whileInView
        <motion.div initial={{ scale: 0.8, opacity: 0 }} whileInView={{ scale: 1, opacity: 1 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.6, ease: "easeInOut" }}>
          <VideoPlayer video={video} playlist={playlist}/>
        </motion.div>) : (<Card className="p-5 overflow-hidden relative">
          ...
          <div className="relative flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            ...
            <div className="shrink-0 flex flex-col gap-2">
              <Button className="h-11 rounded-2xl" onClick={async () => {
                setAccessError(null);
                try {
                    setAccessLoading(true);
                    const res = await api.lessons.play(videoId);
        ...
        <Reveal>
          <Card className="p-4 md:p-5">
            ...
            <div className="mt-3 space-y-2">
              ...
              <textarea value={noteBody} onChange={(e) => setNoteBody(e.target.value)} placeholder="Write a short note..." className="w-full min-h-[140px] rounded-3xl border border-white/10 bg-white/10 px-4 py-3 text-sm font-semibold text-ink-100 shadow-soft focus-ring placeholder:text-ink-400"/>
              <Button variant="secondary" className="h-10 w-full rounded-2xl" onClick={() => {
          ...
          <Reveal delay={0.05}>
            <Card className="p-5">
              ...
              <div className="mt-4 space-y-3">
                {notes.slice(0, 4).map((n) => (<div key={n.id} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 shadow-soft">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <button type="button" className="grid h-9 w-9 place-items-center rounded-2xl border border-white/10 bg-white/10 text-ink-200 shadow-soft transition hover:bg-white/15 focus-ring" onClick={() => setNotes((prev) => prev.filter((x) => x.id !== n.id))} aria-label="Delete note" title="Delete note">
