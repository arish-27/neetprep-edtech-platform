...
function LocalChapterCard({
  ...
  return (
    <Card interactive className="p-5 overflow-hidden">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            {chapter.topics.map((t) => (
              <span
                key={t}
                className="inline-flex items-center gap-1 rounded-lg bg-white/10 border border-white/10 px-2 py-0.5 text-[11px] font-semibold text-ink-300"
              >
        ...
        <div className="mt-4 space-y-1.5">
          {videos.slice(0, 3).map((v, i) => (
            <button
              ...
              className="w-full flex items-center gap-2.5 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-left transition hover:bg-white/10 focus-ring group"
            >
          ...
          {videos.length > 3 && (
            <div className="text-xs text-ink-500 dark:text-ink-400 pl-1">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        <Button
          ...
          onClick={() => navigate(`/app/videos/${chapter.nextVideoId}`)}
        >
        ...
        <Button
          className="h-10 w-full rounded-2xl"
          onClick={() => navigate(`/app/quizzes/${chapter.quizId}/attempt`)}
        >
          Start Quiz
        </Button>
      </div>
...
function ApiCourseCard({
  ...
  course: ApiCoursePublic;
  progressPct: number;
  progressLoading: boolean;
  action: { courseId: string; type: "watch" | "quiz" } | null;
  onAction: (courseId: string, mode: "watch" | "quiz") => void;
}) {
    ...
    <Card interactive className="p-5">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        <Button
          variant="secondary"
          className="h-10 w-full rounded-2xl"
          onClick={() => onAction(course.id, "watch")}
        ...
        <Button
          className="h-10 w-full rounded-2xl"
          onClick={() => onAction(course.id, "quiz")}
...
export function ChaptersScreen() {
  ...
  const apiSubject = subjectId as SubjectKey;
  ...
  const [apiCourses, setApiCourses] = useState<ApiCoursePublic[]>([]);
  ...
  return (
    <div className="space-y-4">
      ...
      <Reveal>
        <Card className="p-5 overflow-hidden">
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <Link
              to="/app/subjects"
              className="text-sm font-bold text-byjus-300 hover:underline inline-flex items-center gap-1 shrink-0"
            >
      ...
      {apiError && showLocalChapters && (
        <div className="rounded-2xl border border-amber-400/30 bg-amber-500/10 px-4 py-3 text-xs font-semibold text-amber-300">
      ...
      {apiLoading && (
        <motion.div
          ...
          animate="show"
        >
          {Array.from({ length: localChapters.length || 3 }).map((_, i) => (
        ...
        >
          {apiCourses.map((c) => (
            <motion.div key={c.id} variants={staggerItem}>
              <ApiCourseCard
                course={c}
                progressPct={courseProgress[c.id] ?? 0}
          ...
          {localChapters.map((ch) => (
            <motion.div key={ch.id} variants={staggerItem}>
              <LocalChapterCard
                chapter={ch}
                subjectId={subjectId}
                progressPct={localChapterProgress[ch.id] ?? ch.progress}
              />
            </motion.div>
          ))}
        </motion.div>
