...
export function RevisionVaultScreen() {
    ...
    const subjectGroups = items.reduce((acc, item) => {
        if (!acc[item.subject])
            acc[item.subject] = [];
    ...
    return (<div className="space-y-5">
      ...
      {loading ? (<div className="flex justify-center py-8"><Loader2 className="h-6 w-6 animate-spin text-byjus-400"/></div>) : items.length === 0 ? (<Card className="p-8 text-center">
        ...
        </Card>) : (Object.entries(subjectGroups).map(([subj, subItems]) => (<Reveal key={subj} delay={0.05}>
            <Card className="p-5">
              ...
              <motion.div className="space-y-3" variants={staggerContainer} initial="hidden" animate="show">
                {subItems.map((item) => (<motion.div key={item.id} variants={staggerItem}>
                    <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          <div className="text-sm font-extrabold text-ink-900 dark:text-ink-50 cursor-pointer" onClick={() => setExpanded(expanded === item.id ? null : item.id)}>
                            {item.question_text}
                          </div>
                          ...
                          {expanded === item.id && item.explanation && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="mt-3 rounded-xl border border-amber-500/20 bg-amber-500/10 px-3 py-2 text-xs font-semibold text-amber-300 overflow-hidden">
                              💡 {item.explanation}
                            </motion.div>)}
                        </div>
                        <Button variant="secondary" className="h-9 rounded-xl shrink-0 text-xs" onClick={() => markReviewed(item.id)} disabled={reviewing === item.id}>
                          {reviewing === item.id
