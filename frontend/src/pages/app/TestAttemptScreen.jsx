...
export function TestAttemptScreen() {
    ...
    useEffect(() => {
        ...
        if (current?.type === "mock_test" && current.questionOrder.length > 0)
            return;
    ...
    const submit = (autoSubmitted) => {
        ...
        for (const id of order) {
            ...
            if (!qu)
                continue;
            if ((answers[id] ?? null) === qu.answerIndex)
                score += 1;
        }
      ...
      <Card className="p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2">
              ...
              <AnimatePresence>
                {showFeedback ? (<motion.span initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }}>
                    <Badge className={cn("border-white/10", isCorrect ? "bg-emerald-500/15 text-emerald-100" : "bg-red-500/15 text-red-100")}>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button variant={marked ? "secondary" : "ghost"} className="h-10 rounded-2xl" onClick={() => toggleMarkForReview(testId, q.id)}>
        ...
        <AnimatePresence mode="wait" initial={false} custom={dir}>
          <motion.div key={q.id} className="mt-5" variants={questionVariants} initial="enter" animate="center" exit="exit" custom={dir} transition={{ duration: 0.22, ease: "easeOut" }}>
            ...
            return (<motion.button key={`${q.id}_${i}`} type="button" onClick={() => answer(testId, q.id, i)} className={cn("w-full text-left rounded-2xl border px-4 py-3 shadow-soft transition focus-ring", cls)} animate={showWrong
                    ? { x: [0, -6, 6, -4, 4, 0] }
                    : showCorrect && chosen
                        ? { scale: [1, 1.02, 1] }
        ...
        <div className="mt-5 flex items-center justify-between gap-2">
          ...
          {idx === total - 1 ? (<Button className="h-11 rounded-2xl" onClick={() => submit(false)}>
              Submit Test
