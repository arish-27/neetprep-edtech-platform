...
import { api, apiSubjectToKey, type ApiDashboardSummary, type ApiQuizResultPublic } from "@/lib/api";
...
function pctForResult(r: ApiQuizResultPublic) {
...
export function PerformanceDashboardScreen() {
  ...
  const [recentQuizzes, setRecentQuizzes] = useState<ApiQuizResultPublic[]>([]);
        ...
        <motion.div variants={staggerItemScale}>
          <Card interactive className="p-5">
            <div className="flex items-center justify-between gap-3">
              ...
              <motion.div className="h-10 w-10 rounded-2xl border border-white/10 bg-white/10 grid place-items-center"
                whileHover={{ scale: 1.15, rotate: 10 }} transition={{ type: "spring", stiffness: 400, damping: 18 }}>
          ...
          <div className="mt-4 space-y-3">
            {quizLoading ? (
              ...
              recentQuizzes.map((r) => {
                ...
                return (
                  <div key={r.id} className="rounded-3xl border border-white/10 bg-white/5 p-4 shadow-soft">
                    <div className="flex items-center justify-between gap-3">
