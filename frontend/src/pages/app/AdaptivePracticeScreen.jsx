...
import { CheckCircle2, Loader2, Sparkles, Trophy, XCircle, Zap } from "lucide-react";
...
export function AdaptivePracticeScreen() {
    ...
    return (<div className="space-y-5 max-w-2xl mx-auto">
      <AnimatePresence mode="wait">
        {phase === "setup" && (<motion.div key="setup" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <Reveal>
              <Card className="p-6">
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {SUBJECTS.map((s) => (<button key={s} type="button" onClick={() => setSubject(s)} className={`rounded-2xl border px-4 py-3 text-sm font-extrabold transition focus-ring ${subject === s ? "border-byjus-500/60 bg-byjus-600/20 text-byjus-300 shadow-glow" : "border-white/10 bg-white/5 text-ink-300 hover:bg-white/10"}`}>
                          {s}
                        </button>))}
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <input value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="e.g. Newton's Laws, Photosynthesis..." className="w-full rounded-2xl border border-white/10 bg-white/10 px-4 py-2.5 text-sm text-ink-100 focus-ring placeholder:text-ink-400"/>
                  </div>
        ...
        {(phase === "question" || phase === "feedback") && currentQ && (<motion.div key="question" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <Card className="p-6">
              {/* Header */}
              <div className="flex items-center justify-between gap-3 mb-5">
                ...
                {feedback && (<div className="flex items-center gap-2">
              ...
              <div className="space-y-2">
                ...
                if (chosen !== null) {
                    if (i === chosen && feedback?.is_correct)
                        cls = "border-emerald-500/50 bg-emerald-500/15 text-emerald-300";
                    else if (i === chosen && !feedback?.is_correct)
                        cls = "border-rose-500/50 bg-rose-500/15 text-rose-300";
                    else
                        cls = "border-white/10 bg-white/5 text-ink-400 opacity-60";
                }
                return (<button key={i} type="button" disabled={chosen !== null} onClick={() => submitAnswer(i)} className={`w-full flex items-center gap-3 rounded-2xl border px-4 py-3 text-sm font-semibold text-left transition focus-ring ${cls}`}>
              ...
              {/* Feedback */}
              {feedback && (<motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-5 space-y-3">
        ...
        {phase === "summary" && summary && (<motion.div key="summary" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}>
            ...
            ].map((s) => (<div key={s.label} className="rounded-2xl border border-white/10 bg-white/5 p-3">
