...
import { Bot, CheckCircle2, HelpCircle, Loader2, MessageSquare, Plus, Sparkles, ThumbsUp } from "lucide-react";
...
export function AskDoubtScreen() {
    ...
    async function getGeminiAnswer(d) {
        if (geminiAnswers[d.id] || geminiLoading === d.id)
            return;
    ...
    return (<div className="space-y-5">
      <Reveal>
        <Card className="p-5">
          ...
          <div className="mt-4 flex gap-2 flex-wrap">
            {SUBJECTS.map((s) => (<button key={s} type="button" onClick={() => setSubject(s)} className={`rounded-2xl border px-3 py-1.5 text-xs font-extrabold transition focus-ring ${subject === s ? "border-byjus-500/60 bg-byjus-600/20 text-byjus-300" : "border-white/10 bg-white/5 text-ink-300 hover:bg-white/10"}`}>
                {s}
              </button>))}
          </div>
        </Card>
      ...
      {showForm && (<Reveal delay={0.05}>
          <Card className="p-5 space-y-4">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {["Physics", "Chemistry", "Biology"].map((s) => (<button key={s} type="button" onClick={() => setAskSubject(s)} className={`rounded-2xl border px-3 py-2 text-xs font-extrabold transition focus-ring ${askSubject === s ? "border-byjus-500/60 bg-byjus-600/20 text-byjus-300" : "border-white/10 bg-white/5 text-ink-300 hover:bg-white/10"}`}>
                      {s}
                    </button>))}
                </div>
              </div>
            ...
            <div className="space-y-2">
              ...
              <textarea value={question} onChange={(e) => setQuestion(e.target.value)} placeholder="Describe your doubt clearly..." className="w-full min-h-[100px] rounded-2xl border border-white/10 bg-white/10 px-4 py-3 text-sm text-ink-100 focus-ring placeholder:text-ink-400"/>
            </div>
      ...
      {loading ? (<div className="flex justify-center py-8"><Loader2 className="h-6 w-6 animate-spin text-byjus-400"/></div>) : doubts.length === 0 ? (<Card className="p-8 text-center">
          ...
          {doubts.map((d) => (<motion.div key={d.id} variants={staggerItem}>
              <Card className="p-5">
                <div className="flex items-start gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    {d.answer && (<div className="mt-3 rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-3">
                    ...
                    {/* Gemini AI Answer */}
                    <div className="mt-3">
                      {geminiAnswers[d.id] ? (<AnimatePresence>
                          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border p-3" style={{ borderColor: "rgba(0,200,83,0.25)", background: "rgba(0,200,83,0.06)" }}>
                        ...
                        </AnimatePresence>) : (<motion.button type="button" onClick={() => getGeminiAnswer(d)} disabled={geminiLoading === d.id} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} className="flex items-center gap-1.5 rounded-xl border px-3 py-1.5 text-xs font-semibold transition focus-ring disabled:opacity-50" style={{ borderColor: "rgba(0,200,83,0.3)", background: "rgba(0,200,83,0.08)", color: "#00C853" }}>
                          {geminiLoading === d.id
                  ...
                  <button type="button" onClick={() => upvote(d.id)} disabled={upvoting === d.id} className="flex flex-col items-center gap-0.5 shrink-0 rounded-xl border border-white/10 bg-white/10 px-2 py-2 text-ink-300 hover:text-byjus-400 transition focus-ring">
