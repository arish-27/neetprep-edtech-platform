...
import { ApiError, api, apiSubjectToKey } from "@/lib/api";
...
function ClassCard({ item, busy, onJoin, }) {
    const gradient = subjectGradient(item.subject);
    return (<Card interactive className="p-4 group overflow-hidden">
      <div className={cn("relative overflow-hidden rounded-3xl border border-white/10 shadow-soft", "bg-gradient-to-br", gradient)}>
...
function VideoModal({ open, item, youtubeId, onClose, }) {
    ...
    const safeResumeSeconds = useMemo(() => {
        ...
        if (!Number.isFinite(approxDuration) || approxDuration <= 0)
            return 0;
    ...
    const commitProgressToServer = useCallback(async (seconds, nextCompleted, opts) => {
        if (!item?.id)
            return;
        ...
        if (!force && now - lastServerCommitRef.current < 4500)
            return;
    ...
    useEffect(() => {
        ...
        if (didInitialSeekRef.current)
            return;
        if (!safeResumeSeconds || safeResumeSeconds < 5)
            return;
        try {
    ...
    return (<AnimatePresence>
      {open && item && safeUrl ? (<motion.div className="fixed inset-0 z-50 grid place-items-center p-3 md:p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <button type="button" className="absolute inset-0 bg-ink-950/70 backdrop-blur-md" onClick={requestClose} aria-label="Close video"/>

          <motion.div className="relative w-full max-w-6xl" initial={{ opacity: 0, y: 14, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.98 }} transition={{ type: "spring", stiffness: 420, damping: 32, bounce: 0.18 }}>
            <Card className="p-4 md:p-5 overflow-hidden">
              ...
              <div className="relative flex items-start justify-between gap-3">
                ...
                <button type="button" className="grid h-10 w-10 place-items-center rounded-2xl border border-white/10 bg-white/10 text-ink-200 shadow-soft transition hover:bg-white/15 focus-ring" onClick={requestClose} aria-label="Close">
                  <X className="h-5 w-5"/>
                </button>
              </div>

              <div className="relative mt-5 grid gap-4 lg:grid-cols-[1.35fr_.65fr]">
                <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-ink-950 shadow-soft" onContextMenu={(e) => e.preventDefault()}>
                  <div className="relative w-full pt-[56.25%]">
                    <div className="absolute inset-0">
                      <ReactPlayer ref={(p) => {
                playerRef.current = p;
            }} key={`${youtubeId}_${playerKey}`} url={safeUrl} playing controls width="100%" height="100%" onReady={() => setReady(true)} onDuration={(d) => {
                if (!Number.isFinite(d))
                    return;
                setDurationSeconds(d);
            }} onProgress={(state) => {
                const now = Date.now();
                if (now - lastCommitRef.current < 1500)
                    return;
                ...
                <div className="space-y-4">
                  <div className="rounded-3xl border border-white/10 bg-white/5 p-4 shadow-soft">
                    ...
                    <div className="mt-4 grid gap-2 text-xs font-semibold text-ink-500 dark:text-ink-300">
                      ...
                      {safeResumeSeconds >= 10 && !completed ? (<div className="flex items-center justify-between">
                          <span>Resume</span>
                  ...
                  <div className="rounded-3xl border border-white/10 bg-white/5 p-4 shadow-soft">
                    ...
                    <div className="mt-3 grid gap-2">
                      ...
                      <Button className="h-11 rounded-2xl" onClick={() => {
                setReady(false);
                setPlayerKey((k) => k + 1);
            }}>
...
export function RecordedClassesScreen() {
    ...
    const tabs = useMemo(() => [{ id: "all", label: "All" }, ...subjects.map((s) => ({ id: s.id, label: s.name }))], []);
    const filtered = useMemo(() => {
        const query = q.trim().toLowerCase();
        return recorded.filter((c) => {
            const key = apiSubjectToKey(c.subject);
            if (subjectId !== "all" && key !== subjectId)
                return false;
            if (!query)
                return true;
    ...
    return (<motion.div className="space-y-4" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, ease: "easeOut" }}>
      ...
      {demoError ? (<Card className="p-4 border border-red-400/30 bg-red-500/10">
      ...
      <Reveal delay={0.05}>
        <Card className="p-5">
          ...
          <motion.div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3" variants={staggerContainer} initial="hidden" animate="show">
            {demoLoading ? (Array.from({ length: 9 }).map((_, i) => (<motion.div key={`sk_rec_${i}`} variants={staggerItem}>
                  <div className="rounded-3xl border border-white/10 bg-white/5 p-4 shadow-soft">
                ...
                </motion.div>))) : filtered.length ? (filtered.map((item) => (<motion.div key={item.id} variants={staggerItem}>
                  <ClassCard item={item} busy={joiningId === item.id} onJoin={joinClass}/>
                </motion.div>))) : (<motion.div variants={staggerItem} className="md:col-span-2 xl:col-span-3">
      ...
      {/* Teacher-uploaded resources */}
      {(resourcesLoading || filteredResources.length > 0) && (<Reveal delay={0.1}>
          <Card className="p-5">
            ...
            {resourcesLoading ? (<div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                {[1, 2, 3].map((i) => (<Skeleton key={i} className="h-16 w-full rounded-2xl"/>))}
              </div>) : (<motion.div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3" variants={staggerContainer} initial="hidden" animate="show">
                {filteredResources.map((r) => (<motion.div key={r.id} variants={staggerItem}>
                    <a href={r.file_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 p-3 shadow-soft transition hover:bg-white/10 focus-ring group">
                      <div className={`h-10 w-10 shrink-0 rounded-xl grid place-items-center ${r.file_type === "video" ? "bg-byjus-600/30" : "bg-amber-500/20"}`}>
                        {r.file_type === "video"
      ...
      <VideoModal open={Boolean(active)} item={active?.item ?? null} youtubeId={active?.youtubeId ?? null} onClose={() => setActive(null)}/>
    </motion.div>);
}
