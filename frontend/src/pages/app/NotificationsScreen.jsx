...
import { starterNotifications } from "@/data/mockData";
export function NotificationsScreen() {
    const [items, setItems] = useLocalStorageState("neet_notifications_v1", starterNotifications);
    const unread = useMemo(() => items.filter((i) => !i.read).length, [items]);
    return (<div className="space-y-4" key="notifications-screen">
      <Reveal>
        <Card className="p-5">
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <Button variant="secondary" className="h-10 rounded-2xl" onClick={() => setItems((prev) => prev.map((x) => ({ ...x, read: true })))} disabled={unread === 0}>
      ...
      <motion.div className="grid gap-4" variants={staggerContainer} initial="hidden" animate="show" key={`notifications-list-${items.length}`}>
        {items.length === 0 ? (<motion.div variants={staggerItem}>
            <Card className="p-6">
          ...
          </motion.div>) : (items.map((n) => (<motion.div key={n.id} variants={staggerItem} layout exit={{ opacity: 0, x: 40, transition: { duration: 0.2 } }}>
              <Card interactive className="p-5">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <div className="truncate text-sm font-extrabold text-ink-900 dark:text-ink-50">{n.title}</div>
                      {!n.read ? (<span className="h-2 w-2 rounded-full bg-byjus-500 dark:bg-byjus-400 shadow-glow"/>) : null}
                    </div>
                  ...
                  <Button variant="ghost" className="h-10 rounded-2xl" onClick={() => setItems((prev) => prev.map((x) => (x.id === n.id ? { ...x, read: true } : x)))}>
                    Mark read
                  </Button>
                </div>
              </Card>
