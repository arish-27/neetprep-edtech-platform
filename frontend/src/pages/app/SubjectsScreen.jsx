...
export function SubjectsScreen() {
    ...
    return (<div className="space-y-4">
      ...
      <motion.div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3" variants={{
            animate: { transition: { staggerChildren: 0.15 } },
        }} initial="initial" animate="animate">
        {filtered.map((s) => {
            const pct = progress[s.id] ?? 0;
            return (<motion.div key={s.id} variants={{ initial: { opacity: 0, y: 60 }, animate: { opacity: 1, y: 0 } }} transition={{ duration: 0.6 }} whileHover={{ scale: 1.05, boxShadow: "0px 0px 20px rgba(0,200,83,0.4)" }}>
              <Link to={`/app/subjects/${s.id}/chapters`} className="focus-ring rounded-3xl block">
                <Card interactive className="p-5 overflow-hidden group">
                  <div className={cn("h-12 w-12 rounded-2xl bg-gradient-to-br shadow-glow grid place-items-center text-white", s.gradient)}>
                  ...
                  <div className="mt-4">
                    <div className="flex items-center justify-between text-sm font-semibold text-ink-700 dark:text-ink-200">
                      <span>Progress</span>
                      {progressLoading ? (<Skeleton className="h-4 w-10"/>) : (<span className="text-ink-500 dark:text-ink-300">{pct}%</span>)}
                    </div>
                    {progressLoading ? (<Skeleton className="mt-2 h-2.5 w-full"/>) : (<ProgressBar value={pct} className="mt-2"/>)}
                  </div>
                </Card>
              </Link>
            </motion.div>);
