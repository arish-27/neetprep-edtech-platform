...
export function QuizScreen() {
    ...
    const openQuiz = async (courseId, dest) => {
        setError(null);
    ...
    return (<div className="space-y-4">
        ...
        <Card className="p-5">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-end">
              ...
              <Button type="button" variant="secondary" className="h-10 rounded-2xl" onClick={startRandomQuiz} disabled={loading || randomBusy} title="Pick a random subject + course and start a quiz">
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
            ...
            return (<button key={t.id} type="button" onClick={() => setSubjectId(t.id)} className={cn("relative h-10 whitespace-nowrap rounded-2xl px-4 text-sm font-extrabold transition focus-ring", active ? "text-ink-50" : "text-ink-200 hover:text-ink-50")}>
                      {active ? (<motion.span layoutId="quiz_tab" className="absolute inset-0 rounded-2xl byjus-gradient opacity-90"/>) : null}
      ...
      {error ? (<Card className="p-4 border border-red-400/30 bg-red-500/10">
          <div className="text-sm font-extrabold text-red-700 dark:text-red-100">{error}</div>
        </Card>) : null}

      <motion.div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3" variants={staggerContainer} initial="hidden" animate="show">
        {loading
            ? Array.from({ length: 6 }).map((_, i) => (<motion.div key={`sk_${i}`} variants={staggerItem}>
                <Card className="p-5">
            ...
            : items.map((c) => {
                ...
                return (<motion.div key={c.id} variants={staggerItem}>
                  <Card interactive className="p-5">
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-2">
                      ...
                      <Button variant="secondary" className="h-10 w-full rounded-2xl" onClick={() => openQuiz(c.id, "result")} disabled={busy}>
                        Results
                      </Button>
                    </div>
                  </Card>
      ...
      {!loading && items.length === 0 ? (<Card className="p-6 text-center">
