    ...
    return (<div className="space-y-4">
      <Reveal>
        <Card className="p-5">
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <Button variant="secondary" className="h-10 rounded-2xl" onClick={clearBookmarks} disabled={items.length === 0}>
              Clear all
            </Button>
          </div>
        </Card>
      ...
      <motion.div className="grid gap-4 md:grid-cols-2" variants={staggerContainer} initial="hidden" animate="show">
        {items.map((b) => {
            ...
            return (<motion.div key={b.key} variants={staggerItem}>
              <Card interactive className="p-5">
                <div className="flex items-start gap-3">
                  {/* Thumbnail / icon */}
                  <div className="h-14 w-24 shrink-0 overflow-hidden rounded-2xl border border-white/10 bg-white/5">
                    {thumb ? (<ThumbImage src={thumb}/>) : (<div className="h-full w-full grid place-items-center bg-white/5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {b.type === "video" && (<div className="mt-1 text-xs text-ink-500 dark:text-ink-400">{b.teacher}</div>)}
                  </div>

                  {/* Remove button */}
                  <button type="button" className="grid h-10 w-10 place-items-center rounded-2xl border border-white/10 bg-white/10 text-ink-200 shadow-soft transition focus-ring hover:bg-white/15" onClick={() => {
                    if (b.type === "video")
                        toggleVideoBookmark(b.refId);
                    else
                        toggleQuestionBookmark(b.refId, b.title);
      ...
      {items.length === 0 && (<Card className="p-6 text-center">
