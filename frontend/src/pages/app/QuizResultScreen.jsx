...
export function QuizResultScreen() {
    ...
    const accuracyPct = typeof location?.state?.accuracyPct === "number" ? location.state.accuracyPct : null;
    ...
    return (<div className="space-y-4">
      <motion.div initial={{ opacity: 0, scale: 0.97, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.35, ease: [0.16, 1, 0.3, 1] }}>
      <Card className="p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              {localAttempt ? (<Badge className="bg-white/10 border-white/10 text-ink-200"><Timer className="h-3.5 w-3.5"/> {formatSeconds(localAttempt.timeTakenSeconds)}</Badge>) : (<Badge className="bg-white/10 border-white/10 text-ink-200"><Timer className="h-3.5 w-3.5"/> n/a</Badge>)}
          ...
          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2, type: "spring", stiffness: 260, damping: 22 }} className="shrink-0">
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-2">
          ...
          {resultsError ? (<div className="text-xs font-semibold text-red-700 dark:text-red-100">{resultsError}</div>) : loadingResults ? (<div className="text-xs font-semibold text-ink-500 dark:text-ink-300">Loading attempts…</div>) : (<div className="text-xs font-semibold text-ink-500 dark:text-ink-300">
              Select an attempt to review.
            </div>)}
        </div>

        <div className="mt-4 flex flex-wrap gap-2">
          {(localAttempts.length ? localAttempts.slice(0, 6).map((a) => ({ kind: "local", id: a.attemptId, score: a.score, total: a.total })) : []).map((a, i) => {
            const active = a.id === selectedKey;
            const pct = a.total ? Math.round((a.score / a.total) * 100) : 0;
            return (<button key={`local_${a.id}`} type="button" onClick={() => setSelectedKey(a.id)} className={cn("h-10 rounded-2xl border px-3 text-xs font-extrabold shadow-soft transition focus-ring", active
                    ? "border-byjus-300 bg-byjus-600 text-white"
                    : "border-white/10 bg-white/10 text-ink-200 hover:bg-white/15")} title="Local review (this device)">
          ...
          {results.slice(0, 6).map((r, i) => {
            ...
            return (<button key={r.id} type="button" onClick={() => setSelectedKey(r.id)} className={cn("h-10 rounded-2xl border px-3 text-xs font-extrabold shadow-soft transition focus-ring", active
                    ? "border-byjus-300 bg-byjus-600 text-white"
                    : "border-white/10 bg-white/10 text-ink-200 hover:bg-white/15")} title={new Date(r.created_at).toLocaleString()}>
      ...
      {localAttempt ? (<motion.div className="grid gap-4" variants={staggerContainer} initial="hidden" animate="show">
          {localAttempt.questionOrder.map((questionId, i) => {
                ...
                return (<motion.div key={questionId} variants={staggerItem}>
              <Card className="p-5">
                ...
                <div className="mt-3 grid gap-2 md:grid-cols-2">
                  {q.options.map((opt, idx) => {
                        ...
                        return (<div key={`${questionId}_${idx}`} className={cn("rounded-2xl border px-4 py-3 shadow-soft", pill)}>
