...
export function SignupScreen() {
    ...
    return (<AuthFrame title="Create account" subtitle="Start your NEET prep journey with structured learning and practice.">
      <form className="space-y-4" onSubmit={async (e) => {
        ...
        <div className="space-y-2">
          ...
          <Input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="student@example.com" type="email" required/>
        </div>
        <div className="space-y-2">
          <label className="text-xs font-extrabold text-ink-700 dark:text-ink-200">Password</label>
          <Input value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Create a strong password" type="password" required/>
        </div>
