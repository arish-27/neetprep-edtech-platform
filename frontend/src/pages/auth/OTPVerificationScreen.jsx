...
export function OTPVerificationScreen() {
    ...
    return (<AuthFrame title="Verify OTP" subtitle={pendingOtpTarget ? `Enter the OTP sent to ${masked}` : "Enter the OTP to continue."}>
      <form className="space-y-4" onSubmit={async (e) => {
        ...
        <div className="space-y-2">
          ...
          <Input value={code} onChange={(e) => setCode(e.target.value)} placeholder="123456" inputMode="numeric" maxLength={6} required/>
        ...
        <div className="flex items-center justify-between gap-3 text-sm font-semibold">
          ...
          <button type="button" className="text-byjus-300 hover:underline" onClick={async () => {
            if (!pendingOtpTarget)
                return;
