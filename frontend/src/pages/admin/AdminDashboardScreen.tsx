...
import { api, subjectKeyToApi, type ApiCoursePublic, type ApiStudentProgressAdmin, type SubjectKey, type UploadFileType } from "@/lib/api";
    ...
    <div>
      ...
      <div className="mt-2">
        <motion.button
          type="button"
          className={cn(
            "w-full rounded-3xl border border-white/10 bg-white/5 p-4 text-left shadow-soft transition focus-ring",
            drag ? "bg-white/10 shadow-neon" : "hover:bg-white/10",
          )}
        ...
        {file ? (
          <div className="mt-2">
            <button
              type="button"
              className="inline-flex items-center gap-2 text-xs font-extrabold text-ink-300 hover:text-ink-100 focus-ring rounded-xl px-2 py-1"
              onClick={() => onFile(null)}
            >
...
export function AdminDashboardScreen() {
  const [students, setStudents] = useState<ApiStudentProgressAdmin[]>([]);
  ...
  const [subjectId, setSubjectId] = useState<string>(subjects[0]?.id ?? "physics");
  ...
  useEffect(() => {
    let cancelled = false;
    const key = subjectId as SubjectKey;
  ...
  const startUpload = async () => {
    ...
    const subject = subjectKeyToApi(subjectId as SubjectKey);
    const uploadOne = async (file: File, fileType: UploadFileType) => {
    ...
    try {
      if (pdfFile) await uploadOne(pdfFile, "pdf");
      if (videoFile) await uploadOne(videoFile, "video");
      setProgress(100);
      setUploading(false);
      setSuccess(true);
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="p-5">
        ...
        {studentsError ? (
          <div className="mt-5 rounded-3xl border border-red-400/30 bg-red-500/10 p-5 shadow-soft">
        ...
        {studentsLoading ? (
          <div className="mt-5 grid gap-4 lg:grid-cols-2">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={`sk_${i}`} className="rounded-3xl border border-white/10 bg-white/5 p-5 shadow-soft">
          ...
          <motion.div className="mt-5 grid gap-4 lg:grid-cols-2" variants={staggerContainer} initial="hidden" animate="show">
            {students.map((s) => {
      ...
      <Card className="p-5" id="admin_upload">
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          <DropZone
            ...
            hint="PDF only (stored locally under /static on the API)."
          ...
          <DropZone
            label="Upload Video"
            hint="MP4 / MOV (stored locally under /static on the API)."
            accept="video/*"
            file={videoFile}
            onFile={setVideoFile}
          />
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-3">
          <div className="space-y-2">
            ...
            <select
              className="h-11 w-full rounded-2xl border border-white/10 bg-white/10 px-4 text-sm font-semibold text-ink-100 shadow-soft focus-ring"
              value={subjectId}
        ...
        <div className="mt-6 rounded-3xl border border-white/10 bg-white/5 p-4 shadow-soft">
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {tags
                  ...
                  .slice(0, 8)
                  .map((t) => (
                    <Badge key={t} className="bg-white/10 border-white/10 text-ink-200">
                      {t}
                    </Badge>
                  ))}
              </div>
          ...
          {uploadError ? (
            <div className="mt-4 rounded-3xl border border-red-400/30 bg-red-500/10 p-4 shadow-soft">
          ...
          <AnimatePresence>
            {success ? (
              <motion.div
                className="mt-4 rounded-3xl border border-emerald-400/30 bg-emerald-500/10 p-4 shadow-soft"
