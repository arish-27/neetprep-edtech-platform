...
import { apiV2, type LiveClass } from "@/lib/apiV2";

const SUBJECTS = ["Physics", "Chemistry", "Biology"];

function statusColor(s: string) {
  if (s === "live") return "bg-emerald-500/20 border-emerald-500/30 text-emerald-300";
  if (s === "ended") return "bg-slate-100 border-slate-200 text-slate-400";
...
export function LiveClassSchedulerScreen() {
  ...
  async function schedule() {
    if (!title.trim() || !startsAt) { setError("Title and start time are required."); return; }
    setError(null);
    setSaving(true);
    try {
      ...
      setTitle(""); setDescription(""); setMeetLink(""); setStartsAt(""); setDurationMin(60);
      setShowForm(false);
    } catch (err: any) {
      ...
      <Reveal delay={0.1}>
        {loading ? (
          ...
          <motion.div className="space-y-3" variants={staggerContainer} initial="hidden" animate="show">
            {classes.map((lc) => (
              <motion.div key={lc.id} variants={staggerItem}>
                    ...
                    <div className="flex flex-col gap-2 shrink-0">
                      {lc.meet_link && (
                        <a href={lc.meet_link} target="_blank" rel="noopener noreferrer"
                          className="flex items-center gap-1 rounded-xl border border-slate-200 bg-slate-100 px-3 py-1.5 text-xs font-extrabold text-slate-400 hover:text-slate-800 transition focus-ring">
                          <ExternalLink className="h-3.5 w-3.5" /> Join
                        </a>
                      )}
                      {lc.status === "scheduled" && (
                        <button type="button" onClick={() => updateStatus(lc.id, "live")}
                          className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-3 py-1.5 text-xs font-extrabold text-emerald-300 hover:bg-emerald-500/20 transition focus-ring">
                          Go Live
                        </button>
                      )}
                      {lc.status === "live" && (
                        <button type="button" onClick={() => updateStatus(lc.id, "ended")}
                          className="rounded-xl border border-slate-200 bg-slate-100 px-3 py-1.5 text-xs font-extrabold text-slate-500 hover:bg-slate-100 transition focus-ring">
                          End
                        </button>
                      )}
                    </div>
