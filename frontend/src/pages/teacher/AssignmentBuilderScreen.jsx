...
export function AssignmentBuilderScreen() {
    ...
    return (<div className="space-y-5">
      ...
      {success && (<div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm font-semibold text-emerald-300">
      ...
      {tab === "create" && (<Reveal delay={0.05}>
          <Card className="p-5 space-y-4">
            ...
            {questions.length > 0 && (<div className="space-y-2">
                ...
                {questions.map((q, i) => (<div key={q.id} className="flex items-start gap-2 rounded-xl border border-slate-200 bg-slate-50 p-3">
      ...
      {tab === "list" && (<Reveal delay={0.05}>
          {listLoading ? (<div className="flex justify-center py-8"><Loader2 className="h-6 w-6 animate-spin text-brand-500"/></div>) : assignments.length === 0 ? (<Card className="p-8 text-center">
              ...
              {assignments.map((a) => (<motion.div key={a.id} variants={staggerItem}>
