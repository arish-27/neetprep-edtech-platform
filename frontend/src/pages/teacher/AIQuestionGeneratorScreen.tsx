...
// Local type matching Gemini output
type GeminiQuestion = {
  id: string;
...
function QuestionCard({
  ...
  const diffColor =
    q.difficulty <= 2 ? "text-emerald-400" :
      ...
      <Card className="p-5">
        ...
        <div className="mb-3 flex items-start justify-between gap-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {q.topic && (
              <Badge className="border-slate-200 bg-slate-100 text-[10px] text-slate-500">
                {q.topic}
              </Badge>
            )}
          ...
          <div className="flex shrink-0 gap-1.5">
            <button type="button" onClick={copy} title="Copy"
              className="grid h-8 w-8 place-items-center rounded-xl border border-slate-200 bg-slate-100 text-slate-500 transition hover:text-slate-800 focus-ring">
              {copied
                ? <CheckCircle2 className="h-3.5 w-3.5 text-emerald-400" />
                : <Copy className="h-3.5 w-3.5" />}
            </button>
            <button type="button" onClick={onRemove} title="Remove"
              className="grid h-8 w-8 place-items-center rounded-xl border border-slate-200 bg-slate-100 text-slate-500 transition hover:text-rose-400 focus-ring">
...
export function AIQuestionGeneratorScreen() {
  ...
  async function generate() {
    ...
    if (!trimmed) { setError("Please select or type a topic first."); return; }
    setError(null);
    setLoading(true);
    try {
      ...
      const raw = await generateNEETQuestions(trimmed, subject, count, diffLabel as any);
      const mapped: GeminiQuestion[] = raw.map((q, i) => ({
      ...
      if (mapped.length === 0) throw new Error("Gemini returned no questions. Try a different topic.");
      setQuestions(mapped);
    } catch (err: any) {
    ...
    <div className="space-y-5">
      <Reveal>
        <Card className="p-5">
          ...
          <div className="grid gap-5 md:grid-cols-2">
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              <TopicAutocomplete
                value={topic}
                subject={subject}
                placeholder="e.g. Electrostatics, Genetics, Kinematics…"
                onChange={setTopic}
                onSelect={setTopic}
              />
              {topic.trim() && (
              ...
              <div className="grid grid-cols-3 gap-2">
                {SUBJECTS.map((s) => (
                  <button key={s} type="button"
                    ...
                    className={`rounded-2xl border px-3 py-2.5 text-xs font-extrabold transition focus-ring ${
                      subject === s
              ...
              <div className="flex gap-2">
                {[3, 5, 8, 10].map((n) => (
                  <button key={n} type="button" onClick={() => setCount(n)}
                    className={`flex-1 rounded-2xl border py-2.5 text-sm font-extrabold transition focus-ring ${
                      count === n
                        ? "border-brand-400 bg-brand-50 text-brand-400 shadow-card"
                        : "border-slate-200 bg-slate-50 text-slate-500 hover:bg-slate-100"
                    }`}>
          ...
          {/* Error */}
          {error && (
            <div className="mt-4 rounded-2xl border border-red-400/30 bg-red-500/10 px-4 py-3 text-sm font-semibold text-red-300">
              {error}
            </div>
          )}
