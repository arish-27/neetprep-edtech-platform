...
export function QuestionBankScreen() {
    ...
    const filtered = questions.filter((q) => !search || q.question_text.toLowerCase().includes(search.toLowerCase()) || q.topic.toLowerCase().includes(search.toLowerCase()));
    ...
    async function importAI() {
        ...
        try {
            const toAdd = aiQuestions.map((q) => ({
                ...
                explanation: q.explanation, difficulty: q.difficulty, tags: "", source: "ai",
            }));
    ...
    return (<div className="space-y-5">
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            ...
            <div className="flex gap-2">
              {["bank", "add", "import"].map((t) => (<button key={t} type="button" onClick={() => setTab(t)} className={`rounded-2xl border px-3 py-1.5 text-xs font-extrabold transition focus-ring capitalize ${tab === t ? "border-brand-400 bg-brand-50 text-brand-400" : "border-slate-200 bg-slate-50 text-slate-500 hover:bg-slate-100"}`}>
      ...
      {tab === "bank" && (<>
            ...
            <Card className="p-4">
              <div className="flex flex-wrap gap-3">
                ...
                <div className="flex gap-2 flex-wrap">
                  {["All", ...SUBJECTS].map((s) => (<button key={s} type="button" onClick={() => setFilterSubject(s)} className={`rounded-2xl border px-3 py-2 text-xs font-extrabold transition focus-ring ${filterSubject === s ? "border-brand-400 bg-brand-50 text-brand-400" : "border-slate-200 bg-slate-50 text-slate-500 hover:bg-slate-100"}`}>
                      {s}
                    </button>))}
                </div>
              </div>
          ...
          {loading ? (<div className="flex justify-center py-8"><Loader2 className="h-6 w-6 animate-spin text-brand-500"/></div>) : filtered.length === 0 ? (<Card className="p-8 text-center">
            ...
            </Card>) : (<motion.div className="space-y-2" variants={staggerContainer} initial="hidden" animate="show">
              {filtered.map((q) => (<motion.div key={q.id} variants={staggerItem}>
                  <Card className="p-4">
                    <div className="flex items-start gap-3">
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-2 grid grid-cols-2 gap-1">
                          {q.options.map((o, i) => (<div key={i} className={`text-xs px-2 py-1 rounded-lg ${i === q.correct_index ? "text-emerald-300 bg-emerald-500/10" : "text-slate-400"}`}>
                              {String.fromCharCode(65 + i)}. {o}
                            </div>))}
                        </div>
                      </div>
                      <button type="button" onClick={() => deleteQ(q.id)} className="grid h-7 w-7 shrink-0 place-items-center rounded-xl border border-slate-200 bg-slate-100 text-slate-400 hover:text-rose-400 transition focus-ring">
      ...
      {tab === "add" && (<Reveal delay={0.05}>
          <Card className="p-5 space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <textarea value={qText} onChange={(e) => setQText(e.target.value)} placeholder="Enter question text..." className="w-full min-h-[80px] rounded-2xl border border-slate-200 bg-slate-100 px-4 py-3 text-sm text-ink-100 focus-ring placeholder:text-slate-400"/>
            </div>
            <div className="space-y-2">
              <label className="text-xs font-extrabold text-slate-400">Options (click to mark correct)</label>
              {opts.map((o, i) => (<div key={i} className="flex gap-2">
                  <button type="button" onClick={() => setCorrectIdx(i)} className={`h-9 w-9 shrink-0 rounded-xl border text-xs font-extrabold transition focus-ring ${correctIdx === i ? "border-emerald-500/50 bg-emerald-500/20 text-emerald-300" : "border-slate-200 bg-slate-100 text-slate-400"}`}>
          ...
          <Card className="p-5 space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-1.5">
                <label className="text-xs font-extrabold text-slate-400">Subject</label>
                <div className="grid grid-cols-3 gap-2">
                  {SUBJECTS.map((s) => (<button key={s} type="button" onClick={() => { setSubject(s); setAiTopic(""); }} className={`rounded-2xl border px-3 py-2 text-xs font-extrabold transition focus-ring ${subject === s ? "border-brand-400 bg-brand-50 text-brand-400" : "border-slate-200 bg-slate-50 text-slate-500 hover:bg-slate-100"}`}>
                      {s}
                    </button>))}
                </div>
              </div>
              <div className="space-y-1.5">
                ...
                <TopicAutocomplete value={aiTopic} subject={subject} placeholder="e.g. Electrostatics, Genetics…" onChange={setAiTopic} onSelect={setAiTopic}/>
              </div>
            </div>

            <Button className="w-full h-11 rounded-2xl" onClick={generateAI} disabled={aiLoading || !aiTopic.trim()}>
              {aiLoading
                ? <><Loader2 className="h-4 w-4 animate-spin"/> Generating…</>
                : <><Sparkles className="h-4 w-4"/> Generate Questions for "{aiTopic || "…"}"</>}
            </Button>

            {!aiTopic.trim() && (<p className="text-center text-xs text-slate-500">
                Select a topic above to enable generation
              </p>)}

            {aiQuestions.length > 0 && (<>
                <div className="space-y-2">
                  {aiQuestions.map((q, i) => (<div key={q.id} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                      <div className="text-xs font-extrabold text-slate-800">{i + 1}. {q.question_text}</div>
                      <div className="mt-1 grid grid-cols-2 gap-1">
                        {q.options.map((o, j) => (<div key={j} className={`text-[10px] px-2 py-0.5 rounded ${o.is_correct ? "text-emerald-300" : "text-slate-500"}`}>
