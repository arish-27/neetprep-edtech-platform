...
export function TestCreatorScreen() {
    ...
    async function generateAndAdd() {
        ...
        try {
            ...
            const asBank = res.questions.map((q) => ({
    ...
    function toggleQuestion(q) {
        setSelectedQuestions((prev) => prev.find((x) => x.id === q.id) ? prev.filter((x) => x.id !== q.id) : [...prev, q]);
    }
    ...
    return (<div className="space-y-5">
          ...
          <div className="grid gap-5 lg:grid-cols-[1fr_1.4fr]">
            ...
            <Card className="p-5 space-y-4">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                ...
                {selectedQuestions.length > 0 && (<button type="button" onClick={() => setSelectedQuestions([])} className="text-[10px] text-rose-400 hover:underline">Clear all</button>)}
              </div>
            ...
            <Card className="p-5">
              ...
              {bankLoading ? (<div className="flex justify-center py-6"><Loader2 className="h-5 w-5 animate-spin text-brand-500"/></div>) : bankQuestions.length === 0 ? (<div className="text-sm text-slate-500 text-center py-6">No questions in bank for {subject}. Add some first.</div>) : (<div className="space-y-2 max-h-[500px] overflow-y-auto pr-1">
                  {bankQuestions.map((q) => {
                    const selected = Boolean(selectedQuestions.find((x) => x.id === q.id));
                    return (<button key={q.id} type="button" onClick={() => toggleQuestion(q)} className={`w-full flex items-start gap-2.5 rounded-xl border p-3 text-left transition focus-ring ${selected ? "border-byjus-500/50 bg-brand-50" : "border-slate-200 bg-slate-50 hover:bg-slate-100"}`}>
      ...
      {tab === "list" && (<Reveal delay={0.05}>
          {loading ? (<div className="flex justify-center py-8"><Loader2 className="h-6 w-6 animate-spin text-brand-500"/></div>) : tests.length === 0 ? (<Card className="p-8 text-center">
              ...
              {tests.map((t) => (<motion.div key={t.id} variants={staggerItem}>
                  <Card interactive className="p-4">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex gap-2 shrink-0">
                        <button type="button" onClick={() => togglePublish(t.id)} className="grid h-8 w-8 place-items-center rounded-xl border border-slate-200 bg-slate-100 text-slate-500 hover:text-brand-500 transition focus-ring" title={t.is_published ? "Unpublish" : "Publish"}>
                          {t.is_published ? <EyeOff className="h-4 w-4"/> : <Eye className="h-4 w-4"/>}
                        </button>
                        <button type="button" onClick={() => deleteTest(t.id)} className="grid h-8 w-8 place-items-center rounded-xl border border-slate-200 bg-slate-100 text-slate-500 hover:text-rose-400 transition focus-ring">
