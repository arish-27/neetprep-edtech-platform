...
function accuracyBg(pct: number) {
...
function StudentCard({ student }: { student: ApiStudentSummaryForTeacher }) {
    ...
    <motion.div variants={staggerItem}>
      ...
      >
        ...
        <div className="flex items-center gap-3">
          {/* Avatar */}
          <div className={`h-10 w-10 shrink-0 rounded-2xl grid place-items-center text-sm font-extrabold text-white ${
            student.is_weak ? "bg-rose-600/60" : "bg-byjus-600/60"
          }`}>
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {student.is_weak && (
                <Badge className="bg-rose-500/20 border-rose-500/30 text-rose-300 text-[10px]">
                  Needs Help
                </Badge>
              )}
            </div>
          ...
          >
            ...
            <div className="mt-3 space-y-1">
              ...
              <div className="h-2.5 w-full rounded-full bg-slate-100 overflow-hidden">
                <motion.div
                  className={`h-full rounded-full ${accuracyBg(student.accuracy_pct)}`}
                  initial={{ width: 0 }}
                  animate={{ width: `${Math.min(100, student.accuracy_pct)}%` }}
            ...
            {student.is_weak && (
              <div className="mt-3 flex items-start gap-2 rounded-xl border border-rose-500/30 bg-rose-500/10 p-3">
...
export function TeacherStudentsScreen() {
  ...
  const students = useMemo(() => {
    if (!data) return [];
    let list: ApiStudentSummaryForTeacher[] = data.all_students;
    if (filter === "top") list = data.top_students;
    if (filter === "weak") list = data.weak_students;
    if (q.trim()) {
      const lq = q.trim().toLowerCase();
      list = list.filter(
        (s) => s.username.toLowerCase().includes(lq) || s.email.toLowerCase().includes(lq),
      );
    }
    return list;
  }, [data, filter, q]);
  ...
  const filters: { id: Filter; label: string; count: number; icon: React.ComponentType<any> }[] = [
  ...
  return (
    <div className="space-y-5">
            ...
            {filters.map((f) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 rounded-2xl border px-3 py-2 text-xs font-extrabold transition focus-ring ${
                    filter === f.id
                ...
                >
                  ...
                  {f.label}
                  <span className={`rounded-lg px-1.5 py-0.5 text-[10px] ${
                    filter === f.id ? "bg-byjus-500/30 text-byjus-200" : "bg-slate-100 text-slate-400"
                  }`}>
                    {f.count}
                  </span>
                </button>
      ...
      {students.length === 0 ? (
        ...
        >
          {students.map((s) => (
            <StudentCard key={String(s.user_id)} student={s} />
          ))}
        </motion.div>
      )}
    </div>
