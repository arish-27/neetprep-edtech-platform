...
function Toggle({ label, desc, checked, onChange }) {
    return (<label className="flex items-start justify-between gap-4 cursor-pointer py-3 border-b border-white/5 last:border-0">
      <div>
        <div className="text-sm font-semibold text-slate-800">{label}</div>
        {desc && <div className="text-xs text-slate-500 mt-0.5">{desc}</div>}
      </div>
      <button type="button" onClick={() => onChange(!checked)} className={`relative h-6 w-11 shrink-0 rounded-full transition-colors focus-ring ${checked ? "bg-byjus-500" : "bg-white/20"}`}>
    ...
    return (<div className="space-y-5 max-w-2xl">
        ...
        <Card className="p-5 space-y-4">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <textarea value={bio} onChange={(e) => setBio(e.target.value)} placeholder="Brief introduction for students..." className="w-full min-h-[80px] rounded-2xl border border-slate-200 bg-slate-100 px-4 py-3 text-sm text-ink-100 focus-ring placeholder:text-slate-400"/>
            </div>
          </div>
        </Card>
      </Reveal>
        ...
        <Card className="p-5 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex gap-2">
              {[1, 2, 3, 4, 5].map((d) => (<button key={d} type="button" onClick={() => setDefaultDiff(d)} className={`flex-1 rounded-2xl border py-2 text-xs font-extrabold transition focus-ring ${defaultDiff === d ? "border-brand-400 bg-brand-50 text-brand-400" : "border-slate-200 bg-slate-50 text-slate-400 hover:bg-slate-100"}`}>
                  {DIFF_LABELS[d]}
                </button>))}
            </div>
          </div>
      ...
      <Reveal delay={0.14}>
        <div className="flex items-center gap-3">
          ...
          {saved && (<div className="flex items-center gap-1.5 text-sm font-semibold text-emerald-400">
