...
import { AlertTriangle, Bell, Loader2, Pin, Plus, Trash2 } from "lucide-react";
...
import { apiV2, type Announcement } from "@/lib/apiV2";
...
export function AnnouncementsScreen() {
  ...
  async function post() {
    ...
    try {
      ...
      setTitle(""); setBody(""); setPriority("normal"); setPinned(false); setShowForm(false);
          ...
          <Card className="p-5 space-y-4">
                ...
                <div className="grid grid-cols-4 gap-1.5">
                  {PRIORITIES.map((p) => (
                    <button key={p.value} type="button" onClick={() => setPriority(p.value)}
                      className={`rounded-xl border px-2 py-1.5 text-[10px] font-extrabold transition focus-ring ${priority === p.value ? p.cls : "border-slate-200 bg-slate-50 text-slate-400 hover:bg-slate-100"}`}>
                      {p.label}
                    </button>
                  ))}
                </div>
            ...
            <div className="space-y-1.5">
              ...
              <textarea value={body} onChange={(e) => setBody(e.target.value)} placeholder="Write your announcement..."
                className="w-full min-h-[100px] rounded-2xl border border-slate-200 bg-slate-100 px-4 py-3 text-sm text-ink-100 focus-ring placeholder:text-slate-400" />
            </div>
              ...
              <Card className={`p-5 ${a.pinned ? "border-brand-200" : ""}`}>
                <div className="flex items-start justify-between gap-3">
                  ...
                  <button type="button" onClick={() => del(a.id)}
                    className="grid h-8 w-8 shrink-0 place-items-center rounded-xl border border-slate-200 bg-slate-100 text-slate-400 hover:text-rose-400 transition focus-ring">
