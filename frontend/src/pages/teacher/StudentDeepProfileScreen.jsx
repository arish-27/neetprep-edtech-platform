...
import { Activity, AlertTriangle, ArrowLeft, CheckCircle2, Clock, Loader2, TrendingUp } from "lucide-react";
    ...
    return (<div className="space-y-5 max-w-2xl">
      ...
      <Reveal delay={0.05}>
        <motion.div className="grid grid-cols-2 gap-3 sm:grid-cols-4" variants={staggerContainer} initial="hidden" animate="show">
          {stats.map((s) => {
            const Icon = s.icon;
            return (<motion.div key={s.label} variants={staggerItem}>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <div className="h-2.5 w-full rounded-full bg-slate-100 overflow-hidden">
                <motion.div className={`h-full rounded-full ${student.accuracy_pct >= 75 ? "bg-emerald-500" : student.accuracy_pct >= 50 ? "bg-amber-500" : "bg-rose-500"}`} initial={{ width: 0 }} animate={{ width: `${student.accuracy_pct}%` }} transition={{ duration: 0.6, ease: "easeOut" }}/>
              </div>
            </div>
          </div>
        </Card>
      ...
      {student.is_weak && (<Reveal delay={0.11}>
