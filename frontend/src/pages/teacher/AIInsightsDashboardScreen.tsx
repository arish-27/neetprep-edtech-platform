...
function accuracyBg(p: number) {
  return p >= 75 ? "bg-emerald-500" : p >= 50 ? "bg-amber-500" : "bg-rose-500";
}

type Insight = { type: "positive" | "warning" | "info"; title: string; detail: string };

function generateInsights(data: ApiTeacherDashboard): Insight[] {
...
export function AIInsightsDashboardScreen() {
  ...
  async function fetchGeminiInsight(d: ApiTeacherDashboard) {
    setGeminiLoading(true);
    try {
      const weakTopics = d.weak_students.slice(0, 3).map((s) => s.username);
      const insight = await analyzePerformance(
        Math.round(d.avg_class_score_pct),
        weakTopics,
        d.subject,
      );
  ...
  useEffect(() => {
    if (data) fetchGeminiInsight(data);
  }, [data]);
        ...
        <Card className="p-5" style={{ background: "linear-gradient(135deg, rgba(0,200,83,0.08) 0%, rgba(0,200,83,0.03) 100%)", border: "1px solid rgba(0,200,83,0.2)" }}>
          <div className="flex items-center justify-between gap-3 mb-3">
            <div className="flex items-center gap-2">
              <motion.div
                className="h-8 w-8 rounded-xl byjus-gradient grid place-items-center"
                animate={{ boxShadow: ["0 0 0px rgba(0,200,83,0)", "0 0 14px rgba(0,200,83,0.5)", "0 0 0px rgba(0,200,83,0)"] }}
            ...
            <motion.button
              type="button"
              onClick={() => data && fetchGeminiInsight(data)}
              disabled={geminiLoading}
              whileHover={{ scale: 1.08, rotate: 180 }}
              whileTap={{ scale: 0.92 }}
              transition={{ duration: 0.3 }}
              className="grid h-8 w-8 place-items-center rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 text-slate-400 hover:text-slate-700 disabled:opacity-40 focus-ring"
            >
            ...
            {insights.map((ins, i) => (
              <motion.div key={i} variants={staggerItem}
                className={`flex items-start gap-3 rounded-2xl border p-3 ${
                  ins.type === "positive" ? "border-emerald-500/30 bg-emerald-500/10" :
              ...
              return (
                <div key={b.label} className="flex-1 flex flex-col items-center gap-1">
                  ...
                  <div className="w-full flex items-end" style={{ height: "80px" }}>
                    <motion.div className={`w-full rounded-t-xl ${b.color}`}
              ...
              {data.all_students.slice(0, 10).map((s) => (
                <div key={String(s.user_id)} className="space-y-1">
                  ...
                  <div className="h-2 w-full rounded-full bg-slate-100 overflow-hidden">
                    <motion.div className={`h-full rounded-full ${accuracyBg(s.accuracy_pct)}`}
                      initial={{ width: 0 }} animate={{ width: `${s.accuracy_pct}%` }}
