...
export function DoubtManagementScreen() {
    ...
    return (<div className="space-y-5">
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            ...
            <div className="flex gap-2 flex-wrap">
              {["open", "answered", "all"].map((s) => (<button key={s} type="button" onClick={() => setStatusFilter(s)} className={`rounded-2xl border px-3 py-1.5 text-xs font-extrabold transition focus-ring capitalize ${statusFilter === s ? "border-brand-400 bg-brand-50 text-brand-400" : "border-slate-200 bg-slate-50 text-slate-500 hover:bg-slate-100"}`}>
                  {s}
                </button>))}
            </div>
          </div>
          ...
          <div className="mt-4 flex gap-2 flex-wrap">
            {SUBJECTS.map((s) => (<button key={s} type="button" onClick={() => setSubject(s)} className={`rounded-2xl border px-3 py-1.5 text-xs font-extrabold transition focus-ring ${subject === s ? "border-brand-400 bg-brand-50 text-brand-400" : "border-slate-200 bg-slate-50 text-slate-500 hover:bg-slate-100"}`}>
                {s}
              </button>))}
          </div>
        </Card>
      ...
      {loading ? (<div className="flex justify-center py-8"><Loader2 className="h-6 w-6 animate-spin text-brand-500"/></div>) : doubts.length === 0 ? (<Card className="p-8 text-center">
              ...
              <Card className="p-5">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    {answeringId === d.id && (<div className="mt-3 space-y-2">
                        <textarea value={answerText} onChange={(e) => setAnswerText(e.target.value)} placeholder="Type your answer..." className="w-full min-h-[80px] rounded-2xl border border-slate-200 bg-slate-100 px-3 py-2 text-sm text-ink-100 focus-ring placeholder:text-slate-400"/>
                  ...
                  <div className="flex flex-col gap-2 shrink-0">
                    ...
                    {d.status === "open" && answeringId !== d.id && (<Button variant="secondary" className="h-8 rounded-xl text-xs" onClick={() => { setAnsweringId(d.id); setAnswerText(""); }}>
