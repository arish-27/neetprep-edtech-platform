...
export function LeaderboardControlScreen() {
    const [xpBoard, setXpBoard] = useState([]);
    ...
    function load() {
        ...
        Promise.all([
            ...
            setXpBoard(lb);
            if (dash) {
                const sorted = [...dash.all_students].sort((a, b) => b.accuracy_pct - a.accuracy_pct);
    ...
    return (<div className="space-y-5">
      <Reveal>
        <Card className="p-5">
          ...
          <div className="mt-4 flex gap-2">
            {["class", "global"].map((t) => (<button key={t} type="button" onClick={() => setTab(t)} className={`rounded-2xl border px-4 py-2 text-xs font-extrabold transition focus-ring capitalize ${tab === t ? "border-brand-400 bg-brand-50 text-brand-400" : "border-slate-200 bg-slate-50 text-slate-500 hover:bg-slate-100"}`}>
      ...
      {loading ? (<div className="flex justify-center py-8"><Loader2 className="h-6 w-6 animate-spin text-brand-500"/></div>) : (<Reveal delay={0.05}>
          <Card className="p-5">
            {tab === "class" ? (<>
                <div className="text-sm font-extrabold text-slate-800 mb-4">
                  Class Ranking — by Quiz Accuracy
                </div>
                {classBoard.length === 0 ? (<div className="text-sm text-slate-500 text-center py-6">No student data yet.</div>) : (<motion.div className="space-y-2" variants={staggerContainer} initial="hidden" animate="show">
                    {classBoard.map((s) => (<motion.div key={String(s.user_id)} variants={staggerItem} className={`flex items-center gap-3 rounded-2xl border p-3 ${s.is_weak ? "border-rose-500/20 bg-rose-500/5" : "border-slate-200 bg-slate-50"}`}>
                ...
                {xpBoard.length === 0 ? (<div className="text-sm text-slate-500 text-center py-6">No XP data yet. Students need to use Adaptive Practice.</div>) : (<motion.div className="space-y-2" variants={staggerContainer} initial="hidden" animate="show">
                    {xpBoard.map((e) => (<motion.div key={e.student_id} variants={staggerItem} className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-3">
